...
. Open a terminal and run the following commands to start Keycloak, PostgreSQL, and Keycloak.
+
[source,shell]
----
cd gateway
...
. To prove everything works, you can run `npm run e2e` in the gateway project's directory. This will run a number of end-to-end tests with https://www.protractortest.org/[Protractor].
+
